...
const Homepage: React.FC<IProps> = (props) => {
	...
	const installBadgeLink = newGroupLink ? (
		<Link href={newGroupLink} title="Перейти" target="_blank" rel="noopener noreferrer">
		...
		<Panel id={id}>
			...
			<Group>
				...
				<SimpleCell badge={installBadgeLink}>
					...
					>
						{inGroup ? "Установить в другую группу" : "Установить приложение в группу"}
					</Button>
				</SimpleCell>
			</Group>
			{groupList.length ? (
				<Group>
					<SimpleCell>
						<InfoRow
							header={`Запись осуществлялась в ${groupList.length} групп${genetive(groupList.length)}:`}
						>
							<Button
								size="l"
								mode="primary"
					...
					<GroupListInfo groupListInfo={groupListInfo} currentGroup_id={currentGroup_id} />
				</Group>
			) : null}
		...
		<List>
            ...
            `}
					>
						...
						>
							<Avatar className="inline" src={group.photo_100} alt={group.name} size={24} />
							&nbsp;{group.id}&nbsp;:&nbsp;{group.name}
							<Icon16Link className="inline" />
						</Link>
...
export default inject("homeStore")(observer(Homepage)) as unknown as React.ComponentType<Omit<IProps, "homeStore">>;
