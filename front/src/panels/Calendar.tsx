...
class Calendar extends React.Component<IProps> {
  ...
  PanelHeaderButtonNext = (
    <Avatar size={36} src={this.props.calendarStore.currentUserPhoto_100} />
  )
      ...
      <Observer>
        {() => (
          <>
            ...
            {this.props.calendarStore.detailedUsersInfoData?.map(
              (user, index) => (
  ...
  renderCell = (day: ICurrentCell, dayIndex: number) => {
    const {
      select,
      openDetailedUsersInfo,
      isSmallIcon,
    } = this.props.calendarStore
    const iconSize = isSmallIcon ? 11 : undefined
    const iconSizeClock = isSmallIcon ? 10 : undefined
      ...
      >
        ...
        >
          ...
          {(!!displayUsersLength || !!day.description) && (
            ...
            >
              {displayUsersLength ? (
                <span className="calendar_body_cell_control_count">
                  {displayUsersLength}
                </span>
              ) : null}
              {!this.props.calendarStore.isMobile && (
        ...
        >
          <div className="calendar_body_cell_users">
            {day.displayUsers?.map((user, userIndex) => (
              <div
                key={userIndex}
                className={`calendar_body_cell_item ${
                  user.IsPinned ? 'pinned' : ''
                }`}
              >
                <Avatar
                  src={user.Photo_100}
  ...
  render() {
    ...
    return (
      <Panel id={id}>
        ...
        {(!!detailedUsersInfoData?.length || !!detailedInfoData) &&
          this.detailedUsersInfo}
        {!!snackBarMessage && (
          <Snackbar
...
export default (inject('calendarStore')(
  observer(Calendar),
) as unknown) as React.ComponentType<Omit<IProps, 'calendarStore'>>
