...
class Setting extends React.Component<IProps, IState> {
	...
	PanelHeaderButtonNext = (
		<PanelHeaderButton title="Перейти в календарь" onClick={this.handleNextPanel}>
			<Icon28CalendarOutline />
		</PanelHeaderButton>
	);
		...
		<Observer>
			{() => (
				<div className="row-wrap">
					{this.props.settingStore.dataAllowedWeekDays.map((isBlocked, index) => (
						<Checkbox
							key={index}
							checked={isBlocked}
							onChange={(e) => {
								this.props.settingStore.changeAllowedWeekDays(index, e.target.checked);
							}}
						>
			...
			{() => (
				<>
					...
					<Input
						...
						onChange={(e) => {
							this.props.settingStore.changeBlockedPeriod(0, e.currentTarget.value);
						}}
					...
					<Input
						...
						onChange={(e) => {
							this.props.settingStore.changeBlockedPeriod(1, e.currentTarget.value);
						}}
				...
				{() => (
					<>
						<ActionSheetItem autoclose={false}>
							<span className="calendar_detailedUsersInfo_label">Добавить описание: </span>
							<Input
								className="calendar_detailedUsersInfo_input_text inline"
								type="text"
								value={this.props.settingStore.detailedCellSettingDay?.Description}
								onChange={(e) => {
									this.props.settingStore.changeDetailedCellSettingDescription(e.target.value);
								}}
							...
							<Icon16CancelCircleOutline
								...
								onClick={this.props.settingStore.clearDetailedCellSettingDescription}
							/>
						</ActionSheetItem>
						<ActionSheetItem autoclose={false}>
							<span className="calendar_detailedUsersInfo_label">Добавить рамку: </span>
							<Input
								className="calendar_detailedUsersInfo_input_color inline"
								type="color"
								value={this.props.settingStore.detailedCellSettingDay?.Color}
								onChange={(e) => {
									this.props.settingStore.changeDetailedCellSettingColor(e.target.value);
								}}
								placeholder="Цвет"
								alt="Добавить рамку"
							/>
						...
						<ActionSheetItem autoclose={false}>
							<div className="row-set">
								<Button onClick={this.props.settingStore.saveDetailedCellSetting} size="l" stretched>
									Сохранить
								</Button>
								<Button
	...
	renderCell = (day: ISettingCell, dayIndex: number) => {
		const { selectBlocked, openDetailedCellSetting } = this.props.settingStore;
		return (
			<div
			...
			>
				<div className={`row calendar_body_cell_control ${day.isInnactive ? "innactive" : ""}`}>
					...
					>
						...
						>
							{day.isBlocked ? <Icon16CancelCircleOutline /> : <Icon16AddCircleOutline />}
						</span>
		...
		return (
			<Panel id={id}>
				...
				<Group title="Общие">
					...
					<Cell indicator={dataAccessMember ? "Вкл" : "Выкл"} after={this.SwitchAccessMember}>
					...
					<Cell after={this.BlockedPeriodField}>Ограничить временной промежуток</Cell>
				</Group>

				<Group title="Виджет группы">
					<Cell indicator={dataWidgetEnable ? "Вкл" : "Выкл"} after={this.SwitchWidgetEnable}>
				...
				<Group title="Сохранить настройки">
					<div className="row-set">
						...
						<Button onClick={this.toogleResetAgreementActive} size="l" stretched mode="secondary">
...
export default inject("settingStore")(observer(Setting)) as unknown as React.ComponentType<
	Omit<IProps, "settingStore">
>;
