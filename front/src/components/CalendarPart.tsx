...
export const CalendarWeekNames: React.FC<IPropsCalendarWeekNames> = ({ isMobile }) => (
	<div className="row calendar_daysNames">
		{calendarWeekNames.map((name, index) => (
			<div className="col" key={index}>
				{isMobile ? name.med : name.full}
			</div>
		))}
	</div>
);
...
export const CalendarHeader: React.FC<IPropsCalendarHeader> = ({ currentMonthTitle, prevMonth, nextMonth }) => (
