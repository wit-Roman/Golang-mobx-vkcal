...
export default class Api implements IApi {
  ...
  listenMessage(incomingData: IIncomingData, callbackAction: IcallbackAction) {
    if (
      incomingData?.length !== 4 ||
      incomingData[1] === this.vkData.query_parameters.viewer_id
    )
      return
    ...
    return bridge
      ...
      .then((result) => {
        return result.response
          ? (result.response as Promise<IgroupInfo[]>)
          : Promise.reject()
      })
      .catch((error) => {
        throw error
  ...
  //BACKEND METHODS
  backSendSession() {
    if (!this.vkData.fetchedUser || !this.vkData.query_parameters?.iframeUrl)
    ...
    return this.ajaxMiddleware(apiUrl + uri.Session, 'POST', body) as Promise<
      IResultbackSendSession
    >
  }
  ...
  backGetUsersData() {
    ...
    return this.ajaxMiddleware(
      apiUrl + '/users?g=' + vk_group_id + '&v=' + viewer_id,
    ) as Promise<IdataUsers>
  }
  ...
  backStartListen(callbackAction: IcallbackAction) {
    ...
    this.sseOpen(
      apiUrl +
        ...
        viewer_id +
        '&t=' +
        encodeURIComponent(this.backToken),
      callbackAction,
    )
  }
  ...
  backDeleteSetting() {
    ...
    const url =
      apiUrl + '/savesetting?reset=1&g=' + vk_group_id + '&v=' + viewer_id
  ...
  backEnableWidget() {
    ...
    return this.ajaxMiddleware(url, 'POST', body) as Promise<{
      Enabled: boolean
    }>
  }
    ...
    return fetch(url, xhrOption)
      .then((response) => {
        ...
        if (response.headers.has('Authorization'))
          this.backToken = response.headers.get('Authorization')!
