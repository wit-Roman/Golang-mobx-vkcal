...
class CalendarStore implements ICalendarStore {
	...
	@observable private startOfMonthDate = Date.parse(startOfMonth(Date.now()).toString());
	...
	setData(data: ICalendarData) {
		...
		this.isSelectBlocked = data.SettingAccessMember && !this.api.isSubscriberRights;
	...
	get currentCells() {
		...
		return Array.apply(null, Array(weeksInMonth)).map((_weekPlug) =>
			Array.apply(null, Array(7)).map((_dayPlug, dayIndex) => {
				...
				const displayUsers = this.dataUsersSelectedDates[cacheDay]?.map((viewer_id) =>
					this.dataUsers[viewer_id]
						? {
								...this.dataUsers[viewer_id],
								IsPinned: !!this.dataUsersPinnedDates[cacheDay]?.includes(viewer_id),
						  }
						: {
				...
				return {
					...
					color: this.specialDays[cacheDay]?.Color ? hex2rgba(this.specialDays[cacheDay].Color) : "",
	...
	select = async (item: ICurrentCell) => {
		...
		try {
			...
			if (result) {
				runInAction(() => {
					...
					if (item.isSelected) {
						...
						this.dataUserSelectedDates = this.dataUserSelectedDates.filter((elem) => elem !== date);
						this.dataUsersSelectedDates[date] = this.dataUsersSelectedDates[date]
							? this.dataUsersSelectedDates[date].filter((elem) => elem !== result.Viewer_id)
							: [];
					}
				});
			}
			...
			runInAction(() => {
				this.snackBarMessage = "Время сессии истекло. Обновите страницу";
			});
			throw e;
		}
	};
	...
	changeOnSSE = (incomingData: IIncomingData) => {
		...
		if (typeIncomingData === incomingDataTypes.pinned) {
			runInAction(() => {
				if (isWriting) {
					...
					if (this.dataUsersPinnedDates[date]) {
						this.dataUsersPinnedDates[date] = this.dataUsersPinnedDates[date].filter(
							(pinned_viewer_id) => pinned_viewer_id !== viewer_id
						);
					}
				}
			});
		...
		if (isWriting) {
			...
			if (this.dataUsersSelectedDates[date]) {
				const deletedIndex = this.dataUsersSelectedDates[date].findIndex((elem) => elem === viewer_id);
				if (deletedIndex !== -1) this.dataUsersSelectedDates[date].splice(deletedIndex, 1);
			}
		}
	...
	openDetailedUsersInfo(date: number) {
		...
		this.detailedUsersInfoData = this.dataUsersSelectedDates[date]?.map((viewer_id) =>
			this.dataUsers[viewer_id]
	...
	/*@action.bound
	private listenMessage(incomingData: IIncomingData) {
		const { users, ...dates } = incomingData;

		if (users) Object.assign(this.data.users, users);
		Object.assign(this.data.dates, dates);
	}*/
}
