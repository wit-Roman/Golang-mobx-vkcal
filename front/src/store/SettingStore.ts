...
export default class SettingStore implements ISettingStore {
  ...
  private endOfCurrentDateDay = Date.parse(
  ...
  async init() {
    try {
      ...
      runInAction(() => {
        ...
        this.displayMinStartBlockedPeriod = format(
          startOfDay(this.currentDateNow),
          'yyyy-MM-dd',
        )
        this.displayMaxEndBlockedPeriod = format(
          addYears(this.currentDateNow, 1),
          'yyyy-MM-dd',
        )
      })
    } catch (error) {
  ...
  selectBlocked = (day: ISettingCell) => {
    if (day.isBlocked) {
      this.dataBlockedDays = this.dataBlockedDays.filter(
        (date) => date !== day.cacheDay,
      )
      day.isBlocked = false
    } else {
      if (this.dataBlockedDays.length > 64) {
        this.snackBarMessage =
          'Количество заблокированных дней не должно превышать 64'
        return
      }
  ...
  changeBlockedPeriod(index: number, value: string | number) {
    this.dataBlockedPeriod[index] = index
      ? new Date(value).valueOf()
      : startOfDay(new Date(value)).valueOf()
  }
    ...
    try {
      ...
      runInAction(() => {
        this.snackBarMessage = result
          ? 'Настройки сохранены'
          : 'Ошибка авторизации, обновите страницу'
      })
    } catch (error) {
      runInAction(() => {
  ...
  saveDetailedCellSetting() {
    ...
    if (
      this.detailedCellSettingDay.Description.length > 78 ||
      this.detailedCellSettingDay.Color.length > 30
    ) {
  ...
  async addWidget(check: boolean) {
    ...
    try {
      ...
      const code = (group_id: number) =>
        'return { "title": "Посещения на ' +
        startDateformated +
        ' - ' +
        endDateformated +
        '", "title_counter": ' +
        count +
        ', "more": "Открыть в приложении ", "more_url": "https://vk.com/club' +
        group_id +
        '?w=app' +
        app_id +
        '_-' +
